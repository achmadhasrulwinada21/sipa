...
class Bunga extends CI_Controller {
	...
	public function index()
	{
		...
		if($kodeadmin=='Direktur Operasional dan Umum'){
		   ...
		   'data_bunga' => $this->bungam->GetBunga("where pinjaman =0 AND mengetahuidirekturcheck ='Approve_kadep' order by id_sbunga desc")->result_array(),
  ...
  //              $data = array(
		...
		// );
		 ...
		 $this->template->Display('bunga/data_bunga',$data);
	...
	function disetujuidirut()
	{
		...
		$this->template->display('bunga/disetujuii', $data);
	}
	...
	function direvisikadep()
	{
		...
		$this->template->display('bunga/direvisibung', $data);
	}
...
function ditolak()
	{
		...
		$this->template->display('bunga/ditolakbung', $data);
	}
	...
	function cek()
	{
		...
		$this->template->display('bunga/cek_bung', $data);
	}
...
function kirim_email($id_sbunga)
    {
	  ...
	  $kirim_email = $this->db->get_where('sukubunga',['id_sbunga'=>$id_sbunga])->row();             		
        ...
        $this->session->set_flashdata("sukses", "<div class='alert alert-success'><strong>Update data BERHASIL di lakukan dan dikirim</strong></div>");
           header('location:'.base_url().'Bunga');
        }
        else
        {
	...
	{	$this->load->model('menum');
	   $this->load->model('Pempinjamm');
		...
		$this->template->Display('bunga/add_bunga',$data);
		
	}

	

	function editbunga($kode=0){	
		$data = array(
			'nama' => $this->session->userdata('nama'),	
			'cabang' => $this->session->userdata('cabang'),
			'nama_user' => $this->session->userdata('nama_user'),	
		...
		$for_bankum = array();
		foreach($this->bungam->GetBunga("where id_sbunga = '$kode' ")->result_array() as $bankum){
			$for_bankum[] = $bankum['banku'];
		}
		...
		$data = array(
			...
			'banku' => $this->model->GetBank()->result_array(),
			...
			$this->template->Display('bunga/edit_bunga',$data);
	}
...
if($this->form_validation->run() == TRUE)
        ...
        $upload_foto = $this->upload->data();
			...
			header('location:'.base_url().'Bunga/addbunga');
		}
	}
 ...
 if($_FILES['file_uploadttd']['error'] == 0):
		...
		if($res>=0){
			...
			header('location:'.base_url().'Bunga/kirim_email/'.$id_sbunga.'');
		}else{
		...
		function hapusbunga($id_sbunga = 1){
		$this->load->model('bungam');
