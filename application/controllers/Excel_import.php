		...
		if(isset($_FILES["file"]["name"]))
		{
			...
			foreach($object->getWorksheetIterator() as $worksheet)
			{
				...
				for($row=2; $row<=$highestRow; $row++)
				{
					...
					$kd_jns_brg1 = $worksheet->getCellByColumnAndRow(8, $row)->getValue();
		...
		if(isset($_FILES["file"]["name"]))
		{
			...
			foreach($object->getWorksheetIterator() as $worksheet)
			{
				...
				for($row=2; $row<=$highestRow; $row++)
				{
					...
					$kd_jns_brg2 = $worksheet->getCellByColumnAndRow(6, $row)->getValue();
		...
		<table class="table table-striped table-bordered">
			<tr>
				...
				<th bgcolor="#4682B4" style="text-align:center; font-family: arial; color: white;">Spesifikasi</th>
 				<th bgcolor="#4682B4" style="text-align:center; font-family: arial; color: white;">Kode Tipe Produk</th>
				<th bgcolor="#4682B4" style="text-align:center; font-family: arial; color: white;">Kode Perusahaan</th>
				 <th bgcolor="#4682B4" style="text-align:center; font-family: arial; color: white;">Volume</th>
				 <th bgcolor="#4682B4" style="text-align:center; font-family: arial; color: white;">Harga</th>
				 <th bgcolor="#4682B4" style="text-align:center; font-family: arial; color: white;">ID Satuan</th>
				<th bgcolor="#4682B4" style="text-align:center; font-family: arial; color: white;">Merk</th>
				<th bgcolor="#4682B4" style="text-align:center; font-family: arial; color: white;">Kode Barang</th>
		...
		if(isset($_FILES["file"]["name"]))
		{
			...
			foreach($object->getWorksheetIterator() as $worksheet)
			{
				...
				for($row=2; $row<=$highestRow; $row++)
				{
					...
					$kode_produk3 = $worksheet->getCellByColumnAndRow(0, $row)->getValue();
					$nama_produk3 = $worksheet->getCellByColumnAndRow(1, $row)->getValue();
					$deskripsi3   = $worksheet->getCellByColumnAndRow(2, $row)->getValue();
					$tipe_produk3 = $worksheet->getCellByColumnAndRow(3, $row)->getValue();
					$koper3 = $worksheet->getCellByColumnAndRow(4, $row)->getValue();
					$volume3 = $worksheet->getCellByColumnAndRow(5, $row)->getValue();
					$harga3 = $worksheet->getCellByColumnAndRow(6, $row)->getValue();
					$satuanid3 = $worksheet->getCellByColumnAndRow(7, $row)->getValue();
					$merk3 = $worksheet->getCellByColumnAndRow(8, $row)->getValue();
					$kd_jns_brg3 = $worksheet->getCellByColumnAndRow(9, $row)->getValue();
...
//perusahaan alkes
	...
	function fetchperalkes()
	{
		...
		foreach($data->result() as $row)
		{
			...
			<tr>
				...
				<td>'.$row->principal.'</td>
				<td>'.$row->solo_agent.'</td>
				<td>'.$row->distributor.'</td>
				<td>'.$row->subdistributor.'</td>
   ...
   //perusahaan perdepbang
	function perdepbang()
	{
		$this->template->display('masterperusahaan/excel_importperdepbang');
	}
	
	function fetchperdepbang()
	{
		$data = $this->excel_import_model->selectperdepbang();
		$output = '
		<h3 align="center">Total Data - '.$data->num_rows().'</h3>
		<table class="table table-striped table-bordered">
			<tr>
	...
	function importperdepbang()
	{
		...
		if(isset($_FILES["file"]["name"]))
		{
			...
			$this->excel_import_model->insertperdepbang($data);
			echo 'Data Imported successfully';
		}	
	}
