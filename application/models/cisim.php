...
class CisiM extends CI_Model {
	...
	{
		$data = $this->db->query('select * from tbl_cis '.$where);
		return $data;
	}
   ...
   public function GetEdit($tables, $datas)
	...
	{
		$datas = $this->db->query('select DISTINCT kode_kom from tbl_cis order by kode_kom asc');
		return $datas;
	}
