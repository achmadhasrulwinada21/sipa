...
class mmrekan extends CI_Model {
	...
	public function get_prov($title){
        $this->db->like('s_nama', $title , 'BOTH');
        $this->db->order_by('s_kode', 'ASC');
        $this->db->limit(10);
	...
	public function GetViewRekanan($where= "") 
	{

        $data = $this->db->query('select pabriknama, s_nama from v_prindist ' .$where);
		return $data;
    }
	...
	public function GetObatCatalog($where="")
	{
		$data = $this->db->query('select * from tb_prod_ecatalog ' .$where);
		return $data;
	}
	
	public function Get_Tipe_rekanan($where="")
	{
	...
	function Update($data){
    	$this->db->where('iddistributor',$data['iddistributor']);
