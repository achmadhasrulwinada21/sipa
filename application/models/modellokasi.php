...
public $variable;
	...
	public function hapusaja($kode_lokasi_a)
    {
        $this->db->where('nm_lokasi',$$kode_lokasi_a);
	...
	function v_lokasi()
	{
        return $this->db->query("
                SELECT a.nm_lokasi,a.koders,c.namars,a.id_lokasi,a.status,a.kode_lokasi
                from master_lokasi a
                LEFT JOIN hrd_mst_rs c ON a.koders=c.koders
                ORDER BY id_lokasi desc
                ")->result();
    }

    function v_lokasiimport()
	{
        return $this->db->query("
                SELECT a.nm_lokasi,a.koders,c.namars,a.id_lokasi,a.status,a.kode_lokasi
                from master_lokasi a
                LEFT JOIN hrd_mst_rs c ON a.koders=c.koders
                ORDER BY id_lokasi desc limit 1000
                ")->result();
    }


	public function gettipeproduk($where= "")
	{
	...
	{
		$this->db->insert_batch('master_lokasi', $data);
	}

	  function Updatelokasi($data){
	  $this->db->where('kode_lokasi',$data['kode_lokasi']);
      $this->db->update_batch('master_lokasi',$data);
 ...
 //    	$this->db->update('dtb_evapekrek',$datas);
	...
	$bln=date('m');
	$thn =date('Y');
	$nisa= ($this->session->userdata('koders'));
