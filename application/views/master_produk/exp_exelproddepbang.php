	<section class="content-header">
		<h4>
          <b>IMPORT EXCEL DETAIL PRODUK DEPBANG</b>
        </h4>
    </section>
...
<!-- Main content -->
          ...
          <div class="row">
            <div class="col-md-12">
			 <div class="box box-primary">
			  ...
			  <a style="margin-bottom:3px" href="<?php echo base_url("excel/detail_produkdepbang.xlsx");?>" 
		...
		<form method="post" action="<?php echo base_url("index.php/C_mstproduk/formdepbang"); ?>" enctype="multipart/form-data">
...
<?php
	if(isset($_POST['preview'])){ // Jika user menekan tombol Preview pada form 
		...
		echo "<form method='post' action='".base_url("index.php/C_mstproduk/importdepbang")."'>";
		...
		<tr>
			<th style='text-align:center;'>KODE SUB JENIS PEKERJAAN</th>
		...
		foreach($sheet as $row){ 
			...
			// $koded_prod = $row['A'];
			// $pabrikid = $row['B'];
			$kode_sub_jenis_pekj = $row['A']; // Ambil data 
			$kode_produk = $row['B']; // Ambil data
			$nama_produk = $row['C'];
			$deskripsi = $row['D']; // Ambil data 
			$tipe_produk = $row['E']; // Ambil data 
			$merk = $row['F'];
			
			
			// Cek jika semua data tidak diisi
			if(empty($kode_sub_jenis_pekj) && empty($kode_produk) && empty($nama_produk) && empty($deskripsi) && empty($tipe_produk) && empty($merk))
			...
			if($numrow > 1){
				...
				$kode_sub_jenis_pekj_a = ( ! empty($kode_sub_jenis_pekj))? "" : " "; 
			...
			if(empty($kode_sub_jenis_pekj) && empty($kode_produk) && empty($nama_produk) && empty($deskripsi) && empty($tipe_produk) && empty($merk)){
				...
				echo "<tr>";
				echo "<td".$kode_sub_jenis_pekj_a.">".$kode_sub_jenis_pekj."</td>";
		...
		<?php
			...
			echo " <input type='submit' name='cancel'> <a href='".base_url("index.php/C_mstproduk/formdepbang")."'>  Cancel </a>";
		}
	...
	<br><br>
	  ...
	  <br><br>
		<a style="margin-bottom:3px" href="<?php echo base_url("index.php/C_mstproduk/depbang"); ?>" 
