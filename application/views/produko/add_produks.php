          ...
          <?php
           $a=$prod->tipe_produk;
                ...
                <tr><td width="200">NAMA PRINSIPAL</td><td width="10">:</td><td width="300"><?php echo $prod->nama_pt ?></td>
                   <tr>  <td width="200">NAMA DISTRIBUTOR</td><td width="10">:</td><td width="600"><?php echo $prod->s_kode ?> </td></tr>
                </tr>
                    <!-- <?php// } ?> -->
                 </table>
                  <?php endif;?>
...
<?php
           ...
           <div class="box">
              ...
              <div class="box-body chat" id="chat-box">
                <!-- chat item -->
                <div class="item">
                  <form role="form" action="<?php echo base_url(); ?>produko/savedata" method="POST" enctype="multipart/form-data" id="multiple_select_form">
              ...
              <div class="col-lg-6">
                    ...
                    <input type="hidden" class="form-control" value="<?php echo $prod->pabrik_id ?>" id="" name="pabrikid">
                      ...
                      <label for="">DISTRIBUTOR</label><br>
                        <select id="" name="s_kode" class="chosen" required>
                          <option value="-">pilih distributor</option>
                          <?php foreach($distributor as $row) { ?>
                              <option value="<?php echo $row['s_kode'] ?>"><?php echo $row['s_kode'] ?></option>
                          <?php } ?>
                        </select> <br>   
                    </div> -->
                  				 <?php 
                   ...
                   ?>
                      ...
                      <label for="">NAMA PRODUK (OBAT)</label><br>
                        <select id="obatid" name="obatid" class="chosen" required>
                          <option required></option>
                          <?php foreach($obat as $row) { ?>
                              <option value="<?php echo $row['obatid'] ?>" required><?php echo $row['obatid'] ?> :  <?php echo $row['obatnama'] ?></option>
                          <?php } ?>
                        </select> <br>   
                    </div>
                 <input type="hidden" class="form-control" value="" name="produko" id="" placeholder="harga" required/>
                  <div class="form-group">
                      <label for="">KOMPOSISI</label>
                       <input type="text" class="form-control" value="" name="komposisi" id="" placeholder="komposisi" required/>                 
                    </div> 
                      <div class="form-group" >
                      <label for="">DISTRIBUTOR</label>
                     <input type="text" class="form-control" value="" name="s_kode" id="" placeholder="" />
                     </div>
                 <?php endif;?>
             <?php 
                         $a=$prod->tipe_produk;
                   ...
                   ?>
                    <div class="form-group" >
                      <label for="">NAMA PRODUK (ALUM)</label>
                        <select id="alumid" name="alumpbkid" class="form-control" required>
                          <option value="-">--Pilih Tipe Produk--</option>
                          <?php foreach($alum as $row) { ?>
                              <option value="<?php echo $row['alumid'] ?>"><?php echo $row['alum'] ?></option>
                          <?php } ?>
                        </select>    
                    </div>
          ...
          <?php endif;?>
            ...
            <a href="<?php echo site_url('produko')?>" class="btn"><i class="icon-remove-sign"></i> BATAL </a>
        </div>
                <!-- /.col -->
               </form>
           ...
           <!-- <?php  // foreach($data_abk as $row) { ?> -->
              ...
              <td style="text-align: center;">Rp.<?php echo number_format($row['harga'], 2,',','.'); ?></td>
                     ...
                     <!--  <a style="margin-bottom:3px;" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#modal_edit<?php //echo $row['iddetailprod'];?>"><i class="glyphicon glyphicon-remove"></i></a> -->
                         <a style="margin-bottom:3px;" class="btn btn-info btn-sm" href="<?php echo base_url(); ?>produko/editabks/<?php echo $row['iddetailprod']; ?>/<?php echo $row['pabrikid']; ?>"><i class="glyphicon glyphicon-edit"></i></a>  
                      <a  style="margin-bottom:3px;" onclick="return confirm('Hapus data beserta detail??');" class="btn btn-danger btn-sm" href="<?php echo base_url(); ?>produko/hapusdetail/<?php echo $row['iddetailprod']; ?>/<?php echo $row['koded_prod']; ?>/<?php echo $row['pabrikid']; ?>"><i class="glyphicon glyphicon-remove"></i></a>
                     </td>
