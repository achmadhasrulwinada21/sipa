...
class CI_Cart {
	...
	function _insert($items = array())
	{
		...
		if ( ! isset($items['id']) OR ! isset($items['qty']) OR ! isset($items['price']) OR ! isset($items['name'])  OR ! isset($items['status_pengajuan'])  OR ! isset($items['peruntukan']) OR ! isset($items['nm_lokasi']) OR ! isset($items['tgtjml_guna_perbulan']) OR ! isset($items['trf_perguna']) OR ! isset($items['biaya_kirim'])  OR ! isset($items['includeongkir']))
		{
	...
	function _save_cart()
	{
		...
		foreach ($this->_cart_contents as $key => $val)
		{
			...
			if ( ! is_array($val) OR ! isset($val['price']) OR ! isset($val['qty']) OR ! isset($val['biaya_kirim']))
			{
				continue;
			}

			$total += ($val['price'] * $val['qty'])+$val['biaya_kirim'];
			$items += $val['qty'];
